...
let index = (_dec = Controller("/manager"), _dec2 = GET("/"), _dec3 = POST("/showpass"), _dec4 = POST("/add"), _dec5 = POST("/update"), _dec6 = POST("/delete"), _dec7 = POST("/swtich"), _dec8 = POST("/assginRole"), _dec(_class = (_class2 = class index extends roleBase {
    ...
    async showpass(ctx, next) {
        var { user_type } = ctx.state['userInfo'];
        if (user_type !== 1) {
            return ctx.body = await ctx.error("你没有权限访问");
        }
        let { id } = ctx.request.body;
        if (!id) return ctx.body = await ctx.error("id is must required");
        try {
            let userInfo = await ctx.Model.findOne({ table: "tk_user", where: { id: id }, field: "password" });
            if (!userInfo) {
                return ctx.body = await ctx.error("没有该用户");
            }
            var data = Aes.aesDecrypt(userInfo.password);
            ctx.body = await ctx.success({ data });
        } catch (error) {
            return ctx.body = await ctx.error("server is error");
        }
    }
...
}, (_applyDecoratedDescriptor(_class2.prototype, "findUser", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "findUser"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "showpass", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "showpass"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "add", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "add"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "update", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "update"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "delete", [_dec6], Object.getOwnPropertyDescriptor(_class2.prototype, "delete"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "swtich", [_dec7], Object.getOwnPropertyDescriptor(_class2.prototype, "swtich"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "assginRole", [_dec8], Object.getOwnPropertyDescriptor(_class2.prototype, "assginRole"), _class2.prototype)), _class2)) || _class);
