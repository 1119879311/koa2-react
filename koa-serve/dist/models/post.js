...
module.exports = class {
    ...
    static async addPost(options = {}) {
        try {
            ...
            var resInsert = await model.thenAdd({
                table: "tk_article",
                values: { title, content: content, cid, thumimg, remark, readcount: 100, status, sort, createtime: new Date().getTime() } }, { title });
    ...
    */
    static async updateAll(options = {}) {
        ...
        try {
            var res = await model.thenUpdate({ table: "tk_article", where: { id }, values: { title, content, cid, sort, thumimg, remark, status } }, { id: ["!=", id], title });
            // 如果title 存在 提示 
    ...
    static async uploadThum(ctx) {
        ...
        var res = await uploadFile(ctx, { maxFieldsSize: 10 * 1024 }, { dir });
        try {
            var files = res.files;
    ...
    static async uploadUeImg(ctx) {
        try {
            ...
            return await { status: true, "errno": 0, "data": resUrl };
        } catch (error) {
            console.log(error);
            return await { status: false, "errno": 1, "data": [] };
        }
    }
};
