...
module.exports = class {
    ...
    static async getUserRole(uid=""){
        try {
            ...
            field:"r.id as role_id,r.name as role_name,r.title as role_title,r.role_type,r.pid as role_pid,r.status as role_staus",
    ...
    static async add(options={}){
        var { name,role_type=1,title,status=1,sort=100} = options;
        var values = { name,title,role_type,status,sort,pid:1,createtime:new Date().getTime(), updatetime:new Date().getTime() };
        try {
    ...
    static async update(options={}){
        ...
        try {
            ...
            if(resInsert.type=="exist") return {errmsg:"name or title is already exist"};
    ...
    static async delete(id){
        try {
            ...
            var resM = await model.delete({table:"tk_menu_role",where:{r_id:id},build:true});  
            var resT = await model.delete({table:"tk_role",where:{id},build:true});      
            // 执行事务（原子性）：
            await model.transaction([resU,resR,resM,resT]);
    ...
    static async assginAuth(r_id,authArrId=[]){
        try {
            ...
            if(authArrId&&authArrId.length){
                var values = authArrId.map(itme=>{return {a_id:itme,r_id} });
    ...
    /** 给角色分配菜单
        * @param {string|Number} r_id 角色id
        * @param {array} menuArrId 行为id  [1,2,3]
    */ 
        static async assginMenu(r_id,menuArrId=[]){
            try {
                // 先删除原来的菜单 
                var sqlArr = [await model.delete({table:"tk_menu_role",where:{r_id},build:true})];
                // 如果有添加menu
                if(menuArrId&&menuArrId.length){
                    var values = menuArrId.map(itme=>{return {m_id:itme,r_id} });
                    sqlArr.push( await model.add({table:"tk_menu_role",values,build:true}) )
                }
                // 执行事务（原子性）：
                await model.transaction(sqlArr);
                return await {mssage:"分配成功"};
