...
const {signRonder,Aes} = imports("util/heper")
...
module.exports = class {
   ...
   static async findAllUser(options={}){ 
       ...
       try {
            ...
            field:"u.id,u.name,u.contact,u.user_type,u.status,u.createtime,group_concat(concat_ws('-',r.id,r.title,r.status) separator '，') as role",
    ...
    static async findOneUser(id=""){
        ...
        // 查用户
       try {
            var res = await model.findOne({table:"tk_user as u", field:"u.id,u.name,u.contact,u.user_type,u.status,u.createtime",where:{id}});
    ...
    static async add(options={}){
        let {name,password,user_type=3,status=1,contact} =  options;
        user_type = user_type===1?3:user_type;
        var values  = { name,status,contact,user_type,
    ...
    static async assginRole(u_id,roleArrId=[]){
        try {
            ...
            if(roleArrId&&roleArrId.length){
                var values = roleArrId.map(itme=>{return {r_id:itme,u_id} });
