...
class connect{
    constructor(dbconfig={}){
        ...
        this.debug = dbconfig.debug||false;
                    ...
                    var resSQL = sqlArr.map( val =>  {
                        return new Promise((resolve,reject)=>{   
                            if(sqlParse.isString(val)){
                                ...
                                var resComs= coms.query(val,(err,res,fields)=>{
                                    if(err) {
                            ...
                            var resComs= coms.query(val[0],val[1],(err,res,fields)=>{
                                    if(err) {
                                    return  coms.rollback(()=>{ reject(err) })
                                    };
                                    resolve(res);
                            ...
                            var resComs=   coms.query(val['sql'],val['value'],(err,res,fields)=>{
                                    if(err) {
                                    return  coms.rollback(()=>{ reject(err) })
                                    };
                                    resolve(res);
